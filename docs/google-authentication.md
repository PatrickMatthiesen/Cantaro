...
Google sign-in uses ASP.NET's standard Google handler. It builds the callback URI from the request scheme, host, path base, and configured callback path. Behind an HTTPS-terminating proxy, preserve the public `Host`, send `X-Forwarded-Proto: https`, and configure the API's immediate proxy in `CANTARO_TRUSTED_PROXY_IP`. Forwarded headers run before authentication. `Frontend:HttpsBaseUrl` (`CANTARO_HTTPS_BASE_URL` in deployment) controls frontend return URLs; it does not override the Google callback URI.

When Cloudflare Tunnel routes through nginx, the API trusts nginx's address. Nginx should trust `CF-Connecting-IP` only from the tunnel connector and overwrite `X-Forwarded-For` with the resolved client address. If nginx sets `X-Forwarded-Proto` from `$scheme`, the tunnel must connect to nginx over HTTPS. Apply the same routing to the login request and Google callback.
