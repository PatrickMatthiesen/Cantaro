...
For Aspire and GitHub deployment, `TrustedProxyIp` / the Production environment secret `CANTARO_TRUSTED_PROXY_IP` accepts a comma-separated list of explicit IPv4 or IPv6 addresses. Configure only the immediate proxies that connect to the API. When both LAN and Cloudflare Tunnel traffic pass through nginx at `192.168.1.151`, use `192.168.1.151`; the API does not need to trust the tunnel connector separately. Whitespace and empty entries are ignored, duplicates are removed, and invalid addresses or CIDR ranges fail startup. An empty value disables forwarded-header trust. Each indexed backend configuration entry also accepts this format.
