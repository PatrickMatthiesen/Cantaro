...
| --- | --- | --- |
| `MediaObservationMatchingService` | Series title, provider series/season IDs, season and episode numbers, observed episode numbers | Search canonical titles, apply provider season mappings, check identity compatibility, and infer cumulative episode offsets. |
...
duplicated in another request body. Repeated series titles use the main observed
title as their fallback. Playback position, duration, watch percentage and unused
...
its evidence and retention window. Once evidence expires, matching requires a
fresh observation; the extension has no offline replay queue.

Account export includes the requesting user's retained observation data. Account
