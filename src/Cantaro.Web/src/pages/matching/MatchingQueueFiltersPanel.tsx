...
function RetryButton({ count, isRetrying, onRetry }: { count: number; isRetrying: boolean; onRetry: () => void }) {
  if (isRetrying) return <GradientButton disabled onClick={onRetry}>Queueing…</GradientButton>;
  return <GradientButton disabled={count === 0} onClick={onRetry}>Retry filtered ({count})</GradientButton>;
}
  ...
  return (
    <section className="border-y border-border-subtle py-4">
      <div className="grid gap-3 md:grid-cols-2 xl:grid-cols-4">
        <FilterSelect label="Status" value={textValue(filters.status)} options={[["", "All unresolved"], ["pending", "Pending"], ["ambiguous", "Ambiguous"], ["no_match", "No match"]]} onChange={(value) => update({ status: value || undefined })} />
      ...
      <div className="mt-4 flex flex-wrap items-center justify-between gap-3">
        ...
        <div className="flex items-center gap-3">
          ...
          <RetryButton count={filteredCount} isRetrying={isRetrying} onRetry={onRetry} />
        </div>
      </div>
    </section>
  );
