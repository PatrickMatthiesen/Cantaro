...
import { mediaApi, mediaProviderCatalog, type MediaProviderAccountStatusDto } from '@cantaro/client-shared/media';
...
function Toggle({ checked, onChange, title, detail }: { checked: boolean; onChange: (checked: boolean) => void; title: string; detail: string }) {
  return (
    <label className="settings-row flex cursor-pointer items-center justify-between gap-5 border-b py-4 last:border-0">
...
function ConnectionCard({ name, detail, icon, implemented, connected, busy, onAction }: { name: string; detail: string; icon: ReactNode; implemented: boolean; connected: boolean; busy: boolean; onAction: () => void }) {
  return (
    <article className="settings-row flex items-center gap-4 rounded-3xl border p-4">
      <span className="flex h-12 w-12 items-center justify-center rounded-2xl bg-white text-xl shadow-sm">{icon}</span>
      ...
      {implemented ? <button type="button" disabled={busy} onClick={onAction} className={`rounded-xl px-4 py-2 text-xs font-black transition ${connected ? 'bg-emerald-50 text-emerald-700 hover:bg-rose-50 hover:text-rose-700' : 'bg-violet-600 text-white hover:bg-violet-700'}`}>{busy ? 'Working…' : connected ? 'Connected' : 'Connect'}</button> : <span className="rounded-full bg-slate-100 px-3 py-1.5 text-[10px] font-black tracking-wider text-slate-500 uppercase">Soon</span>}
    </article>
  );
}
...
function MusicConnectionCard({ item, status, busyKey, onAction }: { item: (typeof platformCatalog)[number]; status?: PlatformAccountStatus; busyKey: string | null; onAction: () => void }) {
  ...
  return <ConnectionCard name={item.name} detail={detail} icon={<MusicPlatformIcon platformId={item.id} className="h-6 w-6" />} implemented={item.implemented} connected={connected} busy={busyKey === `music-${item.id}`} onAction={onAction} />;
}
...
function MediaConnectionCard({ item, status, busyKey, onAction }: { item: (typeof mediaProviderCatalog)[number]; status?: MediaProviderAccountStatusDto; busyKey: string | null; onAction: () => void }) {
  ...
  return <ConnectionCard name={item.name} detail={detail} icon={<span>{item.icon}</span>} implemented={item.implemented} connected={connected} busy={busyKey === `media-${item.id}`} onAction={onAction} />;
}
...
function PreferencesSections({ profile, onProfile }: { profile: User; onProfile: (profile: User) => void }) {
  ...
  return <>
    <SettingsSection id="notifications" eyebrow="Signal, not noise" title="Notification defaults" description="Choose which events should become in-app notices as Cantaro's notification center comes online.">
      <div className="rounded-3xl border px-5"><Toggle checked={preferences.notifyOnSyncSuccess} onChange={value => change('notifyOnSyncSuccess', value)} title="Successful syncs" detail="Let me know when a playlist finishes syncing." /><Toggle checked={preferences.notifyOnSyncFailure} onChange={value => change('notifyOnSyncFailure', value)} title="Sync failures" detail="Surface provider errors and syncs that need attention." /><Toggle checked={preferences.notifyOnMediaReview} onChange={value => change('notifyOnMediaReview', value)} title="Media review queue" detail="Flag new observations that need a match decision." /></div>{saveFooter}
    </SettingsSection>
