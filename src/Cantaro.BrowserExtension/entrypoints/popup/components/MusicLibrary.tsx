...
function SongDetail({ song, playlists, onBack }: { song: MusicLibrarySong; playlists: MusicLibraryResponse['playlists']; onBack: () => void }) {
  ...
  return <article className="space-y-3 p-1"><button onClick={onBack} className="rounded-lg px-2 py-1 text-xs font-bold text-accent-strong hover:bg-surface-hover">← Back to music</button><div className="flex gap-3 rounded-2xl bg-surface p-3">{song.thumbnailUrl ? <img src={song.thumbnailUrl} alt="" className="size-20 rounded-xl object-cover" /> : <div className="size-20 rounded-xl bg-accent-soft" />}<div className="min-w-0"><h2 className="text-lg font-black">{song.title}</h2><p className="text-sm text-content-muted">{song.artist || 'Unknown artist'}</p><p className="mt-1 text-xs text-content-muted">{song.albums.join(', ') || 'No album'}{song.durationSeconds ? ` · ${Math.floor(song.durationSeconds / 60)}:${String(song.durationSeconds % 60).padStart(2, '0')}` : ''}</p></div></div>
    ...
    {song.sourceIdentities.length > 0 ? <Section title="Identifiers">{song.sourceIdentities.map((identity) => <p key={`${identity.source}-${identity.externalId}`} className="truncate text-xs text-content-muted"><b>{identity.source}:</b> {identity.externalId}</p>)}</Section> : null}
    {youtubeIds.length > 1 ? <Section title="YouTube version"><select value={selectedYouTubeId} onChange={(event) => setSelectedYouTubeId(event.target.value)} className="w-full rounded-lg border border-border-strong bg-surface px-2 py-1.5 text-xs text-content"><option value="">Choose a version to sync…</option>{youtubeIds.map((id) => <option key={id} value={id}>{id}</option>)}</select></Section> : null}
    <Section title="Playlists"><div className="mb-2 flex items-center justify-between gap-3"><p className="text-xs text-content-muted">Tracked playlist memberships</p>{trackId && availablePlaylists.length ? <button onClick={() => setShowPicker((value) => !value)} className="flex size-7 items-center justify-center rounded-full bg-accent-soft text-lg font-bold text-accent-strong hover:bg-surface-hover" aria-label="Add to another playlist">+</button> : null}</div>{memberships.length ? <div className="space-y-1">{memberships.map((playlist) => <div key={playlist.playlistId} className="group flex min-h-9 items-center gap-2 rounded-lg px-2 text-sm text-content hover:bg-danger-surface"><span className="min-w-0 flex-1 truncate">{playlist.playlistName} <span className="text-xs text-content-subtle">#{playlist.position + 1}</span></span>{confirmingRemoval === playlist.playlistId ? <div className="flex items-center gap-1"><span className="text-[11px] font-semibold text-rose-700">Remove?</span><button disabled={busy !== null} onClick={() => void removeFromPlaylist(playlist.playlistId)} className="rounded bg-rose-700 px-2 py-1 text-[11px] font-bold text-white">Confirm</button><button onClick={() => setConfirmingRemoval(null)} className="rounded px-2 py-1 text-[11px] font-bold text-content-muted">Cancel</button></div> : <button onClick={() => setConfirmingRemoval(playlist.playlistId)} className="flex size-7 items-center justify-center rounded-md text-rose-600 opacity-0 transition hover:bg-rose-100 group-hover:opacity-100 focus:opacity-100" aria-label={`Remove from ${playlist.playlistName}`}><span aria-hidden>🗑</span></button>}</div>)}</div> : <p className="text-xs text-content-muted">Not currently in a playlist.</p>}{showPicker ? <div className="mt-3 flex flex-wrap gap-2 border-t border-border-subtle pt-3">{availablePlaylists.map((playlist) => <button key={playlist.id} disabled={busy !== null} onClick={() => void addToPlaylist(playlist)} className="rounded-lg bg-accent-soft px-3 py-1.5 text-xs font-bold text-accent-strong hover:bg-surface-hover disabled:opacity-50">{busy === playlist.id ? 'Adding…' : playlist.name}</button>)}</div> : null}{message ? <p className="mt-2 text-xs text-content-muted" role="status">{message}</p> : null}</Section>
  </article>;
}
...
function Section({ title, children }: { title: string; children: React.ReactNode }) { return <section className="rounded-2xl bg-surface-translucent p-3"><h3 className="mb-2 text-xs font-black tracking-wider text-content-muted uppercase">{title}</h3>{children}</section>; }
